<script>
	...
	function startAgentSession(topic) {
		const topicId = topic.index;
		const title = encodeURIComponent(topic.improved_topic_name || topic.original_topic);
		// Note the maths-agent=true parameter
		goto(`/?topic=${topicId}&title=${title}&maths-agent=true`);
	}
</script>
...
<div class="h-full w-full flex flex-col p-6 overflow-y-auto bg-gray-50 dark:bg-gray-900">
	<div class="max-w-6xl mx-auto w-full">
		<header class="mb-10 text-center">
			<h1
				class="text-4xl font-extrabold mb-3 text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-indigo-600 dark:from-purple-400 dark:to-indigo-400"
			>
				MIRA Maths Agent
			</h1>
			<p class="text-lg text-gray-600 dark:text-gray-400">
				The advanced agentic math helper. Powered by precise reasoning and tool-calling.
			</p>
		</header>
			...
			<div class="animate-fade-in-up">
				<div class="flex items-center justify-between mb-6 px-2">
					<h2 class="text-xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
						<span
							class="bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400 text-xs px-2.5 py-1 rounded-full uppercase tracking-wider"
						>
							Grade {selectedGrade}
						</span>
						Topics
				...
				{#if loading}
					<div class="flex flex-col items-center justify-center py-20 text-gray-500">
						<div
							class="w-8 h-8 border-3 border-purple-500 border-t-transparent rounded-full animate-spin mb-4"
						></div>
						<span class="font-medium animate-pulse">Loading agent curriculum...</span>
					</div>
				...
				{:else}
					<div class="space-y-4">
						{#each topics as topic, i}
							<div
								class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-purple-200 dark:hover:border-purple-800
                                {expandedTopicIndex === i
									? 'ring-2 ring-purple-500/20 dark:ring-purple-500/30 shadow-md'
									: ''}"
							>
								<!-- Card Header / Summary -->
								<div
								...
								>
									<!-- Index Badge -->
									<div
										class="flex-shrink-0 w-10 h-10 rounded-full bg-purple-50 dark:bg-purple-900/20 flex items-center justify-center text-purple-600 dark:text-purple-400 font-bold text-sm"
									>
										{i + 1}
									</div>
									...
									<div class="flex-grow">
										<div class="flex justify-between items-start">
											<h3
												class="font-bold text-lg text-gray-900 dark:text-gray-100 mb-1 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors"
											>
												{topic.original_topic} : {topic.improved_topic_name || topic.original_topic}
											</h3>
											<button
												class="text-gray-400 hover:text-purple-500 transition-transform duration-300 {expandedTopicIndex ===
												i
													? 'rotate-180'
													: ''}"
											>
								...
								{#if expandedTopicIndex === i}
									...
									>
										...
										<div class="px-6 pb-6 pt-2 flex justify-end">
											<button
												class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2.5 rounded-xl font-medium shadow-lg shadow-purple-500/30 transition-all hover:scale-105 flex items-center gap-2"
												on:click={(e) => {
													e.stopPropagation();
													startAgentSession(topic);
												}}
											>
