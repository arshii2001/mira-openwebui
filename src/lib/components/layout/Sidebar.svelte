...
{#if !$mobile && !$showSidebar}
	<div
		class=" pt-[7px] pb-2 px-2 flex flex-col justify-between text-black dark:text-white hover:bg-[var(--mira-sidebar-bg)]/30 dark:hover:bg-[var(--mira-sidebar-bg-dark)]/30 h-full z-10 transition-all border-e-[0.5px] border-gray-100 dark:border-gray-850/30"
		id="sidebar"
	>
		<button
		...
		>
			...
			<div class="-mt-[0.5px]">
				...
				<div>
					<Tooltip content={$i18n.t('Learn')} placement="right">
						<a
							class=" cursor-pointer flex rounded-xl hover:bg-gray-100 dark:hover:bg-gray-850 transition group"
							href="/learn"
							on:click={async (e) => {
				...
				<div>
					<Tooltip content={$i18n.t('Classroom')} placement="right">
						<a
							class=" cursor-pointer flex rounded-xl hover:bg-gray-100 dark:hover:bg-gray-850 transition group"
							href="/classroom"
							on:click={async (e) => {
				...
				<div>
					<Tooltip content="Maths" placement="right">
						<a
							class=" cursor-pointer flex rounded-xl hover:bg-gray-100 dark:hover:bg-gray-850 transition group"
							href="/maths"
							on:click={async (e) => {
						...
						>
							<div class=" self-center flex items-center justify-center size-9">
								...
								<svg
									...
									class="size-4.5"
								>
									<!-- Σ: top-right → top-left → middle-right → bottom-left → bottom-right -->
...
{#if $showSidebar}
	<div
		...
		id="sidebar"
		class="h-screen max-h-[100dvh] min-h-screen select-none font-mira {$showSidebar
			? `${$mobile ? 'bg-[var(--mira-sidebar-bg)] dark:bg-[var(--mira-sidebar-bg-dark)]' : 'bg-[var(--mira-sidebar-bg)]/70 dark:bg-[var(--mira-sidebar-bg-dark)]/70'} z-50`
	...
	>
		...
		>
			<div
				class="sidebar px-[0.5625rem] pt-2.5 pb-1.5 flex justify-between space-x-1 text-gray-600 dark:text-gray-400 sticky top-0 z-10 -mb-3"
			>
				<a
					class="flex items-center gap-2 rounded-xl h-full justify-center hover:bg-gray-100/50 dark:hover:bg-gray-850/50 transition no-drag-region px-1.5 py-1"
					href="/"
					draggable="false"
				...
				>
					...
					<div
						...
						class="self-center font-bold mira-gradient-text text-lg tracking-wide"
					>
						MIRA
					</div>
				</a>
				...
				<div
					class="{scrollTop > 0
						? 'visible'
						: 'invisible'} sidebar-bg-gradient-to-b bg-linear-to-b from-[var(--mira-sidebar-bg)] dark:from-[var(--mira-sidebar-bg-dark)] to-transparent from-50% pointer-events-none absolute inset-0 -z-10 -mb-6"
				></div>
			</div>
			...
			>
				<div class="pb-1.5">
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						<a
							id="sidebar-new-chat-button"
							class="group grow mira-sidebar-item"
							href="/"
							draggable="false"
							on:click={newChatHandler}
							aria-label={$i18n.t('New Chat')}
					...
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						...
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class=" self-center text-sm">{$i18n.t('Learn')}</div>
							</div>
						</a>
					</div>
					...
					<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
						...
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class=" self-center text-sm">{$i18n.t('Classroom')}</div>
							</div>
						</a>
					</div>
					...
					{#if $user?.role === 'admin' || $user?.permissions?.workspace?.models || $user?.permissions?.workspace?.knowledge || $user?.permissions?.workspace?.prompts || $user?.permissions?.workspace?.tools}
						<div class="px-[0.4375rem] flex justify-center text-gray-800 dark:text-gray-200">
							...
							>
								...
								<div class="flex self-center translate-y-[0.5px]">
									<div class=" self-center text-sm">{$i18n.t('Workspace')}</div>
								</div>
							</a>
						</div>
					{/if}
			...
			<div class="px-1.5 pt-1.5 pb-2 sticky bottom-0 z-10 -mt-3 sidebar">
				<div
					class=" sidebar-bg-gradient-to-t bg-linear-to-t from-[var(--mira-sidebar-bg)] dark:from-[var(--mira-sidebar-bg-dark)] to-transparent from-50% pointer-events-none absolute inset-0 -z-10 -mt-6"
				></div>
